...
void RPLLSpellHooks::DoDamageAndTriggers(const Spell *spell, const uint32 hitMask)
{
    ...
    const RPLL_DamageHitMask damageHitMask = RPLLHooks::mapSpellCastHitMaskToRPLLHitMask(hitMask);

    const auto unitTarget = spell->m_targets.GetUnitTarget();
    if (unitTarget != nullptr)
    {
        RPLLHooks::SpellCast(caster->ToUnit(), unitTarget->GetGUID().GetRawValue(), spell->GetSpellInfo()->Id, damageHitMask);
        return;
    }
    ...
    {
        RPLLHooks::SpellCast(caster->ToUnit(), 0, spell->GetSpellInfo()->Id, damageHitMask);
        return;
    }

    // Assume self cast
    RPLLHooks::SpellCast(caster->ToUnit(), caster->GetGUID().GetRawValue(), spell->GetSpellInfo()->Id, damageHitMask);
}