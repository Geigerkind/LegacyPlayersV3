...
impl RetrieveCharacterSkills for ArmoryExporter {
  fn get_profession_skills(&self, db_characters: &mut impl Select, character_id: u32) -> Vec<CharacterSkillTable> {
    db_characters.select_wparams("SELECT skill, value, max FROM character_skills \
      WHERE guid=:character_id AND skill IN (164,165,171,182,186,197,202,333,393,755,773)", move |mut row| CharacterSkillTable {
      character_id,
