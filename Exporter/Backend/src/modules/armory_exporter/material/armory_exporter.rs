...
impl ArmoryExporter {
    pub fn init(mut self, db_lp_consent: &mut impl Select) -> Self
    {
        ...
        self.last_fetch_time = db_lp_consent.select_value("SELECT last_fetch FROM meta_data", |mut row| {
            let last_fetch: u64 = row.take(0).unwrap();
            last_fetch
        }).unwrap();

        self.last_instance_reset_fetch_time = db_lp_consent.select_value("SELECT last_instance_reset_fetch FROM meta_data", |mut row| {
            let last_instance_reset_fetch: u64 = row.take(0).unwrap();
            last_instance_reset_fetch
        }).unwrap();

        db_lp_consent.select_wparams("SELECT enchant_id, item_id FROM data_gem \
      WHERE expansion_id=:expansion_id", |mut row| {
            let enchant_id: u32 = row.take(0).unwrap();
            let item_id: u32 = row.take(1).unwrap();
            (enchant_id, item_id)
        }, params!("expansion_id" => expansion_id)).into_iter()
            .for_each(|(enchant_id, item_id)| { self.gem_enchant_id_to_item_id.insert(enchant_id, item_id); });

        db_lp_consent.select_wparams("SELECT a.tab_order, a.tier, a.column, b.ppt, a.rank0, a.rank1, a.rank2, a.rank3, a.rank4 FROM data_talent a \
      ...
      )).into_iter().for_each(|ranks| ranks.into_iter().for_each(|(rank_id, meta_talent)| { self.spell_id_to_meta_talent.insert(rank_id, meta_talent); }));
