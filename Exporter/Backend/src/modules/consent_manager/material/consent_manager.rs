  ...
  {
    {
      ...
      db_lp_consent.select("SELECT character_id FROM character_consent WHERE ISNULL(consent_withdrawn_when)", |mut row| {
        let character_id: u32 = row.take(0).unwrap();
        character_id
      }).into_iter().for_each(|result| {
      ...
      db_lp_consent.select("SELECT guild_id FROM guild_consent WHERE ISNULL(consent_withdrawn_when)", |mut row| {
