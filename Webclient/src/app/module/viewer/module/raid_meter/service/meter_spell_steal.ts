...
export class MeterSpellStealService implements OnDestroy {
    ...
    private initialize(): void {
        ...
        this.subscription = this.instanceDataService.knecht_updates.subscribe(async ([knecht_update, evt_types]) => {
            if (knecht_update.includes(KnechtUpdates.FilterChanged) || (knecht_update.includes(KnechtUpdates.NewData) && [8].some(evt => evt_types.includes(evt))))
                this.merge_data();
        });
    }
    ...
    private async merge_data(): Promise<void> {
        ...
        const result1 = await this.instanceDataService.knecht_spell_steal.meter_spell_steal(this.current_mode);
