...
export class HealTakenDetailService implements OnDestroy {
    ...
    private initialize(): void {
        ...
        this.instanceDataService.get_heal(true).pipe(take(1)).subscribe(spell_damage => {
        ...
        this.subscription_changed = this.instanceDataService.changed.subscribe(changed => {
            ...
            if ([ChangedSubject.Sources, ChangedSubject.Targets, ChangedSubject.Attempts, ChangedSubject.Heal].includes(changed))
                this.instanceDataService.get_heal(true).pipe(take(1)).subscribe(heal => {
                    this.heal = heal;
                    this.commit();
                });
        });
