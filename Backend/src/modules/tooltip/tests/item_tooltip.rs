...
fn avenger_breastplate() {
    ...
    assert!(item_tooltip
        .bonding
        .contains(&"Binds when picked up".to_string()));
    ...
    assert_eq!(
        item_tooltip.item_set.as_ref().unwrap().name,
    ...
    assert_eq!(
        item_tooltip.item_set.as_ref().unwrap().set_effects[0].threshold,
        3
    );
    assert_eq!(
        item_tooltip.item_set.as_ref().unwrap().set_effects[0].description,
        "Increases the duration of your Judgements by 20%."
    );
    assert_eq!(
        item_tooltip.item_set.as_ref().unwrap().set_effects[1].threshold,
        5
    );
    assert_eq!(
        item_tooltip.item_set.as_ref().unwrap().set_effects[1].description,
...
fn shadowmourne_socket() {
    ...
    assert_eq!(
        item_tooltip.socket.as_ref().unwrap().socket_bonus,
        "+8 Strength"
    );
    ...
    assert_eq!(
        item_tooltip
            ...
            .slots
            .iter()
            .filter(|slot| slot.flag == 2 && slot.item.is_none())
            .count(),
        3
    );
}
...
fn thunderfury_weapon_stat() {
    ...
    assert_eq!(
        item_tooltip.weapon_stat.as_ref().unwrap().damage_sources[0].damage_min,
        44
    );
    assert_eq!(
        item_tooltip.weapon_stat.as_ref().unwrap().damage_sources[0].damage_max,
        115
    );
    assert!(item_tooltip.weapon_stat.as_ref().unwrap().damage_sources[0]
        .damage_type
        .is_none());
    assert_eq!(
        item_tooltip.weapon_stat.as_ref().unwrap().damage_sources[1].damage_min,
        16
    );
    assert_eq!(
        item_tooltip.weapon_stat.as_ref().unwrap().damage_sources[1].damage_max,
        30
    );
    assert!(item_tooltip.weapon_stat.as_ref().unwrap().damage_sources[1]
        .damage_type
        .contains(&"Nature".to_string()));
}
...
fn shadowmourne_socketed_and_enchanted() {
    ...
    let result = tooltip.get_character_item(
        &data,
        &armory,
        1,
        49623,
    ...
    assert!(item_tooltip
        .enchant
        .contains(&"Fishing Lure (+100 Fishing Skill)".to_string()));
    assert_eq!(
        item_tooltip.socket.as_ref().unwrap().socket_bonus,
        "+8 Strength"
    );
    ...
    assert!(item_tooltip.socket.as_ref().unwrap().slots[0]
        .item
        .is_none());
    assert!(item_tooltip.socket.as_ref().unwrap().slots[1]
        .item
        .is_some());
    assert_eq!(
        item_tooltip.socket.as_ref().unwrap().slots[1]
            .item
            .as_ref()
            .unwrap()
            .effect,
    ...
    assert!(item_tooltip.socket.as_ref().unwrap().slots[2]
        .item
        .is_some());
    assert_eq!(
        item_tooltip.socket.as_ref().unwrap().slots[2]
            .item
            .as_ref()
            .unwrap()
            .effect,
