...
use crate::modules::data::tools::{
    RetrieveEnchant, RetrieveGem, RetrieveIcon, RetrieveItem, RetrieveItemBonding,
    RetrieveItemClass, RetrieveItemDamage, RetrieveItemDamageType, RetrieveItemEffect,
    RetrieveItemInventoryType, RetrieveItemRandomProperty, RetrieveItemRandomPropertyPoints,
    RetrieveItemSheath, RetrieveItemSocket, RetrieveItemStat, RetrieveItemsetEffect,
    RetrieveItemsetName, RetrieveLocalization, RetrieveStatType, SpellDescription,
};
use crate::modules::data::Data;
use crate::modules::tooltip::domain_value::{
    ItemSet, SetEffect, SocketSlot, SocketSlotItem, Stat, WeaponDamage, WeaponStat,
};
...
pub trait RetrieveItemTooltip {
    fn get_item(
        ...
        expansion_id: u8,
        item_id: u32,
    ) -> Result<ItemTooltip, TooltipFailure>;
    fn get_character_item(
        &self,
        data: &Data,
        armory: &Armory,
        language_id: u8,
...
impl RetrieveItemTooltip for Tooltip {
    fn get_item(
        ...
        let item_stat_effects = [
            7, 8, 37, 23, 24, 10, 11, 12, 42, 38, 39, 40, 41, 9, 13, 21, 43, 22,
        ];
                    ...
                    ItemSet {
                        ...
                        set_effects: data
                            ...
                            .map(|itemset_effect| SetEffect {
                                ...
                                description: data
                                    .get_localized_spell_description(
                                        expansion_id,
                                        language_id,
                                        itemset_effect.spell_id,
                                    )
                                    .unwrap(),
                            })
        ...
        if item_set.is_some() {
            item_set
                ...
                .unwrap()
                .set_effects
                .sort_by(|left, right| left.threshold.cmp(&right.threshold));
        }
        ...
        if item_stats.is_some() {
            ...
            inner_item_stats
                .iter()
                .filter(|item_stat| item_stat_effects.contains(&item_stat.stat.stat_type))
                .for_each(|item_stat| {
                    if item_effects.is_none() {
                        item_effects = Some(Vec::new());
                    }

                    item_effects.as_mut().unwrap().insert(
                        0,
                        get_item_stat_effect_localization(
                            data,
                            language_id,
                            item_stat.stat.stat_type,
                        )
    ...
    fn get_character_item(
        ...
        let character = self
            .get_character(data, armory, language_id, character_history.character_id)
            .unwrap();
        ...
        if item_tooltip.item_set.is_some() {
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.head,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
            ...
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.chest,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.shirt,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.tabard,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.wrist,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.glove,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.belt,
            ...
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.ring1,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.ring2,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.trinket1,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.trinket2,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.main_hand,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.off_hand,
            );
            check_is_active(
                item_tooltip.item_set.as_mut().unwrap().set_items.as_mut(),
                &character_gear.ternary_hand,
            );
        ...
        if item_tooltip.socket.is_some() {
            try_fill_socket(
                ...
                expansion_id,
                language_id,
                item_tooltip.socket.as_mut().unwrap(),
                &character_gear.head,
                item_id,
            );
            try_fill_socket(
        ...
        try_apply_random_item_property(
            data,
            expansion_id,
            language_id,
            &mut item_tooltip,
...
fn try_apply_enchant(
    data: &Data,
    expansion_id: u8,
    language_id: u8,
    enchant: &mut Option<String>,
...
fn try_apply_random_item_property(
    data: &Data,
    expansion_id: u8,
    language_id: u8,
    item_tooltip: &mut ItemTooltip,
    item: &Option<CharacterItem>,
    item_id: u32,
    suffix_index: u8,
) {
    ...
    let item_random_property = data
        .get_item_random_property(expansion_id, *random_property_id)
        .unwrap();
    let property_suffix = data
        .get_localization(language_id, item_random_property.localization_id)
        .unwrap()
        .content;
    let data_item = data.get_item(expansion_id, item_res.item_id).unwrap();
    let property_points = data_item
        .item_level
    ...
    for i in 0..item_random_property.enchant_ids.len() {
        ...
        let effect_value = data
            .get_localization(language_id, enchant.localization_id)
            .unwrap()
            .content;
        if *random_property_id < 0 {
            ...
            let stat_value =
                ((coefficient_value * scaling_factor as u32) as f64 / 10000.0).floor() as u16;
