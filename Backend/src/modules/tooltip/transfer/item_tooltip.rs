...
pub fn get_item(
    ...
    expansion: Expansion,
    id: u32,
) -> Result<Json<ItemTooltip>, TooltipFailure> {
    me.get_item(&data, language.0, expansion.0, id)
        .and_then(|tooltip| Ok(Json(tooltip)))
}

#[openapi]
#[get("/item/<character_history_id>/<item_id>")]
pub fn get_character_item(
    me: State<Tooltip>,
    data: State<Data>,
    armory: State<Armory>,
    language: Language,
    ...
    me.get_character_item(&data, &armory, language.0, item_id, character_history_id)
        .and_then(|tooltip| Ok(Json(tooltip)))
}
