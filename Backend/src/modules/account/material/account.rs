...
impl Account {
    pub fn init(self) -> Self {
        ...
        member.insert(entry.id, entry);
            ...
            for entry in self.db_main.select(
                "SELECT id, member_id, token, purpose, exp_date FROM account_api_token",
                &|mut row| APIToken {
                    id: row.take(0).unwrap(),
                ...
                api_token_to_member_id
                    .insert(entry.token.as_ref().unwrap().clone(), entry.member_id);
    ...
    fn clean_tokens(&self) {
        self.db_main
            .execute("DELETE FROM account_api_token WHERE exp_date < UNIX_TIMESTAMP()");
    }
}
