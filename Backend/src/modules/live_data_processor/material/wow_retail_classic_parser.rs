...
impl WoWRetailClassicParser {
    ...
    pub fn collect_participant_class(&mut self, unit: &Unit, spell_id: u32) {
        lazy_static! {
            ...
            static ref SHAMAN_SPELLS: BTreeSet<u32> = [15208, 10605, 403, 10392, 26363,].iter().cloned().collect();
            static ref DRUID_SPELLS: BTreeSet<u32> = [1064, 10622, 10623, 10448, 939, 10395, 10468, 25357, 9881, 9908].iter().cloned().collect();
        }
