...
pub trait ProcessMessages {
    fn parse_messages(&self, db_main: &mut (impl Select + Execute), server_id: u32, armory: &Armory, data: &Data, messages: Vec<Vec<u8>>) -> Result<(), LiveDataProcessorFailure>;
...
impl ProcessMessages for LiveDataProcessor {
    fn parse_messages(&self, db_main: &mut (impl Select + Execute), server_id: u32, armory: &Armory, data: &Data, messages: Vec<Vec<u8>>) -> Result<(), LiveDataProcessorFailure> {
        ...
        self.process_messages(db_main, server_id, armory, data, msg_vec)
    }
