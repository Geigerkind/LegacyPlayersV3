...
pub trait ProcessMessages {
    ...
    fn process_messages(&self, db_main: &mut (impl Select + Execute), server_id: u32, armory: &Armory, data: &Data, msg_vec: Vec<Message>, member_id: u32) -> Result<(), LiveDataProcessorFailure>;
}
...
impl ProcessMessages for LiveDataProcessor {
    fn parse_messages(&self, db_main: &mut (impl Select + Execute), server_id: u32, armory: &Armory, data: &Data, messages: Vec<Vec<u8>>, member_id: u32) -> Result<(), LiveDataProcessorFailure> {
        ...
        self.process_messages(db_main, server_id, armory, data, msg_vec, member_id)
    }
    ...
    fn process_messages(&self, db_main: &mut (impl Select + Execute), server_id: u32, armory: &Armory, data: &Data, msg_vec: Vec<Message>, member_id: u32) -> Result<(), LiveDataProcessorFailure> {
        if !msg_vec.is_empty() {
            ...
            return server.parse_events(db_main, armory, data, msg_vec, member_id);
        }
