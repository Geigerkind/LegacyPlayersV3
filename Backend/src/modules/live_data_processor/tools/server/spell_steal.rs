...
use crate::modules::live_data_processor::dto::{Message, MessageType, UnAura};
...
pub fn try_parse_spell_steal(non_committed_messages: &mut Vec<Message>, committed_events: &[Event], timestamp: u64, spell_steal: &UnAura, subject: &Unit, armory: &Armory, server_id: u32, summons: &HashMap<u64, u64>) -> Option<(u32, u32)> {
    ...
    for i in (0..committed_events.len()).rev() {
        ...
        match &event.event {
            EventType::SpellCast(spell_cast) => {
                if let Some(spell_id) = &spell_cast.spell_id {
                    if let Some(victim) = &spell_cast.victim {
                        if victim == &target && &event.subject == subject && is_spell_steal(*spell_id) {
            ...
            EventType::AuraApplication(aura_application) => {
                if event.subject == target && aura_application.caster == aura_caster && aura_application.spell_id == spell_steal.target_spell_id && aura_application.stack_amount > 0 {
        ...
        if let Some(spell_cast_event_id) = spell_cast_event_id {
            if let Some(aura_application_event_id) = aura_application_event_id {
                return Some((spell_cast_event_id, aura_application_event_id));
            }
        }
    }
    ...
    for (msg_index, msg) in non_committed_messages.iter().enumerate() {
        ...
        match &msg.message_type {
            MessageType::SpellCast(spell_cast) => {
                if spell_cast_msg_index.is_none() && spell_cast_event_id.is_none() {
                    if let Some(target) = &spell_cast.target {
                        if target == &spell_steal.target && spell_cast.caster == spell_steal.un_aura_caster && is_spell_steal(spell_cast.spell_id) {
    ...
    } else if let Some(aura_application_index) = aura_application_msg_index {
