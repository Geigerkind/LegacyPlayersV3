            ...
            EventType::SpellCast(spell_cast) => {
                ...
                if let Some(spell_id) = &spell_cast.spell_id {
                    if let Some(victim) = &spell_cast.victim {
                        if victim == &target && &event.subject == subject && dispel.un_aura_spell_id == *spell_id {
    ...
    for (msg_index, msg) in non_committed_messages.iter().enumerate() {
        ...
        match &msg.message_type {
            MessageType::SpellCast(spell_cast) => {
                if !spell_cast_found && spell_cast_event_id.is_none() {
                    if let Some(target) = &spell_cast.target {
                        if target == &dispel.target && spell_cast.caster == dispel.un_aura_caster && spell_cast.spell_id == dispel.un_aura_spell_id {
            ...
            MessageType::AuraApplication(aura_application) => {
                if num_dispels_found == dispel.un_aura_amount as usize && !aura_application.applied && aura_application.caster == dispel.aura_caster && aura_application.target == dispel.target && aura_application.spell_id == dispel.target_spell_id {
                    num_dispels_found += 1;
                    msg_indexes.push(msg_index);
                }
            },
