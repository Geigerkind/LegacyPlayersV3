...
use crate::modules::live_data_processor::domain_value::{AuraApplication, Event, EventType, Position, Power, PowerType, UnitInstance};
use crate::modules::live_data_processor::dto::{CombatState, Death, LiveDataProcessorFailure, Loot, Message, MessageType, Summon, Unit};
...
impl ParseEvents for Server {
    ...
    ///   * The start is only called when it really starts, e.g. there are events up to 1 min that need to be assigned to that instance then
    ...
    /// * Map information is provided through the position of a player, hence we will not always know immediately where to assign the player to.
    /// * For Raids, we have to distinguish between a continuation and a new instance
    ///   * InstanceIds are recycled upon every reset (There is a data structure that provides information about raid reset times)
...
fn extract_committable_event(server: &mut Server, armory: &Armory, server_id: u32) -> Option<Event> {
    ...
    match first_message.message_type {
        // Spell
        MessageType::SpellCast(_) | MessageType::Threat(_) | MessageType::Heal(_) | MessageType::MeleeDamage(_) | MessageType::SpellDamage(_) => {
            event.event = EventType::SpellCast(try_parse_spell_cast(&mut server.non_committed_messages, &server.summons, &first_message, armory, server_id)?)
        },
        ...
        MessageType::CombatState(CombatState { unit: _, in_combat }) => {
            server.non_committed_messages.pop().unwrap();
            event.event = EventType::CombatState { in_combat };
        },
        MessageType::Loot(Loot { unit: _, item_id }) => {
            server.non_committed_messages.pop().unwrap();
            event.event = EventType::Loot { item_id };
        },
        ...
        MessageType::Interrupt(interrupt) => {
            if let Some((cause_event_id, interrupted_spell_id)) = try_parse_interrupt(&mut server.non_committed_messages, &server.committed_events, first_message.timestamp, &interrupt, &subject) {
                event.event = EventType::Interrupt { cause_event_id, interrupted_spell_id };
            } else {
        ...
        MessageType::SpellSteal(spell_steal) => {
            if let Some((cause_event_id, target_event_id)) = try_parse_spell_steal(&mut server.non_committed_messages, &server.committed_events, first_message.timestamp, &spell_steal, &subject, armory, server_id, &server.summons) {
                event.event = EventType::SpellSteal { cause_event_id, target_event_id };
            } else {
                return None;
            }
        },
        MessageType::Dispel(dispel) => {
            if let Some((cause_event_id, target_event_ids)) = try_parse_dispel(&mut server.non_committed_messages, &server.committed_events, first_message.timestamp, &dispel, &subject, armory, server_id, &server.summons) {
                event.event = EventType::Dispel { cause_event_id, target_event_ids };
            } else {
                return None;
            }
        },
        MessageType::Event(event_dto) => {
            ...
            if event_dto.event_type == 0 {
                if let Ok(creature @ domain_value::Unit::Creature(_)) = event_dto.unit.to_unit(armory, server_id, &server.summons) {
                    event.event = EventType::ThreatWipe { creature };
                }
            }
        },
