...
use crate::modules::armory::tools::{
    CreateCharacterFacial, CreateCharacterInfo, CreateGuild, GetCharacter, GetCharacterHistory,
    SetGuildRank,
};
use crate::modules::armory::Armory;

pub trait CreateCharacterHistory {
    fn create_character_history(
        &self,
        server_id: u32,
...
impl CreateCharacterHistory for Armory {
    ...
    fn create_character_history(
        ...
        let character_id = self
            .get_character_id_by_uid(server_id, character_uid)
            .unwrap();
        ...
        if let Some(char_guild_dto) = character_history_dto.character_guild.as_ref() {
            ...
            if let Err(e) =
                self.set_guild_rank(*guild_id.as_ref().unwrap(), char_guild_dto.rank.clone())
            {
                return Err(e);
            }
        }
        let character_info_res =
            self.create_character_info(character_history_dto.character_info.to_owned());
        ...
        Err(ArmoryFailure::Database(
            "create_character_history".to_owned(),
        ))
    }
}
