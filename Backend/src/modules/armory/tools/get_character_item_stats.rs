...
use crate::modules::data::tools::{
    RetrieveEnchant, RetrieveGem, RetrieveItem, RetrieveItemEffect, RetrieveItemRandomProperty,
    RetrieveItemRandomPropertyPoints, RetrieveItemSocket, RetrieveItemStat, RetrieveItemsetEffect,
    RetrieveItemsetName, RetrieveLocalization, RetrieveStatType, SpellDescription,
};
use crate::modules::data::Data;
use crate::modules::data::Stat;

pub fn get_character_stats(
    data: &Data,
    language_id: u8,
    expansion_id: u8,
    gear: &CharacterGear,
    ...
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.shoulder, 1),
    );
    ...
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.tabard, 0),
    );
    ...
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.main_hand, 3),
    );
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.off_hand, 3),
    );
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.ternary_hand, 4),
    );
    ...
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.trinket1, 1),
    );
    merge_character_stat_vec(
        &mut acc,
        get_item_stats(data, expansion_id, &gear.trinket2, 1),
    );
    ...
    for item_id in gear_to_vec_item_ids {
        ...
        if let Some(item) = data.get_item(expansion_id, item_id) {
            if let Some(itemset_id) = item.itemset {
                let mut itemset_item_ids =
                    data.get_itemset_item_ids(expansion_id, itemset_id).unwrap();
                ...
                data.get_itemset_effects(expansion_id, itemset_id)
                    ...
                    .for_each(|item_effect| {
                        merge_character_stat_vec(
                            &mut acc,
                            data.parse_stats(expansion_id, item_effect.spell_id),
                        )
                    })
            }
        }
        ...
        if random_property_id > 0 {
            ...
            for i in 0..random_stats.enchant_ids.len() {
                ...
                let localization = data
                    .get_localization(1, enchant.localization_id)
                    .unwrap()
                    .content
                    .to_lowercase();
