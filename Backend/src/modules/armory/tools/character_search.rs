...
use crate::modules::armory::dto::{
    CharacterSearchCharacterDto, CharacterSearchFilter, CharacterSearchGuildDto,
...
pub trait PerformCharacterSearch {
    fn get_character_search_result(
        &self,
        data: &Data,
    ...
    fn get_character_search_result(
        ...
        if filter.name.filter.is_some() {
            *filter.name.filter.as_mut().unwrap() =
                filter.name.filter.as_ref().unwrap().to_lowercase();
        }
        if filter.guild.filter.is_some() {
            *filter.guild.filter.as_mut().unwrap() =
                filter.guild.filter.as_ref().unwrap().to_lowercase();
        }
        let mut result: Vec<CharacterSearchResult> = characters
            .iter()
            .filter(|(_, character)| character.last_update.is_some())
            .filter(|(_, character)| {
                filter.server.filter.is_none()
                    || filter.server.filter.contains(&character.server_id)
            })
            .filter(|(_, character)| {
                filter.name.filter.is_none()
                    || character
                        .last_update
                        .as_ref()
                        .unwrap()
                        .character_name
                        .to_lowercase()
                        .contains(filter.name.filter.as_ref().unwrap())
            })
            .filter(|(_, character)| {
                filter.hero_class.filter.is_none()
                    || filter.hero_class.filter.contains(
                        &character
                            .last_update
                            .as_ref()
                            .unwrap()
            ...
            .filter(|(_, character)| {
                ...
                if let Some(character_guild) = character
                    .last_update
                    .as_ref()
                    .unwrap()
                    .character_guild
        ...
        result.sort_by(
            |left: &CharacterSearchResult, right: &CharacterSearchResult| {
                if let Some(sorting) = filter.hero_class.sorting {
                    let ordering = left
                        .character
                        .hero_class_id
                        .cmp(&right.character.hero_class_id);
