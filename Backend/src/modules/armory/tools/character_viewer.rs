...
use crate::modules::data::tools::{RetrieveRace, RetrieveItem, RetrieveServer, RetrieveIcon, RetrieveTitle, RetrieveLocalization, RetrieveProfession, RetrieveHeroClass, RetrieveStatType, RetrieveItemStat, RetrieveEnchant, RetrieveGem, RetrieveItemSocket, RetrieveItemEffect, SpellDescription, RetrieveItemsetEffect, RetrieveItemsetName};
...
fn get_item_stats(data: &Data, expansion_id: u8, item: &Option<CharacterItem>) -> Vec<Stat> {
  ...
  if let Some(enchant_id) = item.enchant_id {
    ...
    merge_character_stat_vec(&mut stats, enchant.stats); // TODO: Parse
  }
