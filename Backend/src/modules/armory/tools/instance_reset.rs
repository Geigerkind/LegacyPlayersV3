...
pub trait HandleInstanceReset {
    fn set_instance_resets(&self, db_main: &mut impl Execute, server_id: u32, instance_resets: Vec<InstanceResetDto>) -> Result<(), ArmoryFailure>;
    fn update_cache(&self) -> Result<(), ArmoryFailure>;
}

impl HandleInstanceReset for Armory {
    fn set_instance_resets(&self, _db_main: &mut impl Execute, _server_id: u32, _instance_resets: Vec<InstanceResetDto>) -> Result<(), ArmoryFailure> {
        unimplemented!()
        /*
