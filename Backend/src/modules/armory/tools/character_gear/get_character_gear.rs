...
pub trait GetCharacterGear {
    ...
    fn get_character_gear_by_value(
        &self,
...
impl GetCharacterGear for Armory {
    fn get_character_gear(&self, gear_id: u32) -> Result<CharacterGear, ArmoryFailure> {
        ...
        self.db_main
            .select_wparams_value(
                "SELECT * FROM armory_gear WHERE id=:id",
                &|mut row| {
                    Ok(CharacterGear {
                        ...
                        head: row
                            ...
                            .unwrap()
                            .ok()
                            .and_then(|id| self.get_character_item(id).ok()),
                        neck: row
                            .take_opt(2)
                            .unwrap()
                            .ok()
            ...
            .unwrap_or_else(|| Err(ArmoryFailure::Database("get_character_gear".to_owned())))
    }
    ...
    fn get_character_gear_by_value(
        ...
        let head = gear
            .head
            .as_ref()
            .and_then(|item| self.get_character_item_by_value(item.to_owned()).ok());
        ...
        self.db_main
            .select_wparams_value(
                "SELECT id FROM armory_gear WHERE \
      ...
      AND ((ISNULL(:trinket2) AND ISNULL(trinket2)) OR trinket2 = :trinket2)",
                &|mut row| {
                    Ok(CharacterGear {
                        id: row.take(0).unwrap(),
                        head: head.to_owned(),
            ...
            .unwrap_or_else(|| {
                Err(ArmoryFailure::Database(
                    "get_character_gear_by_value".to_owned(),
                ))
            })
    }
}
