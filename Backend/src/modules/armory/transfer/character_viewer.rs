...
use crate::modules::armory::dto::{ArmoryFailure, CharacterViewerDto};
...
pub fn get_character_viewer_by_history(
    me: State<Armory>,
    data: State<Data>,
    language: Language,
    server_name: String,
    ...
    data.get_server_by_name(server_name)
        ...
        .and_then(|server| {
            me.get_character_by_name(server.id, character_name)
                .ok_or(ArmoryFailure::InvalidInput)
                .and_then(|character| {
                    me.get_character_viewer_by_history_id(
                        &data,
                        language.0,
