...
use crate::modules::armory::domain_value::{
    CharacterFacial, CharacterGear, CharacterGuild, CharacterInfo, CharacterItem, GuildRank,
    HistoryMoment,
};
...
impl Init for HashMap<u32, Character> {
    fn init(&mut self, db: &MySQLConnection) {
        ...
        db.select(
            "SELECT id, timestamp, character_id FROM armory_character_history ORDER BY id",
            &|mut row| {
...
impl Init for HashMap<u32, Guild> {
    fn init(&mut self, db: &MySQLConnection) {
        ...
        db.select(
            "SELECT * FROM armory_guild_rank ORDER BY guild_id, rank_index",
            &|mut row| {
