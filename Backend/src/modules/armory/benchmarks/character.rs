...
*/
#[bench]
fn set_character(bencher: &mut Bencher) {
    ...
    bencher.bench(|| {
        let _ = armory.set_character(3, character_dto.clone());
    });

    // Cleanup
    let character_history = armory.get_character_by_uid(3, character_dto.server_uid).unwrap().last_update.unwrap();
