...
impl RetrieveTinyUrl for Utility {
    fn get_tiny_url(&self, db_main: &mut impl Select, id: u32) -> Result<TinyUrl, UtilityFailure> {
        db_main.select_wparams_value("SELECT id, url_payload FROM utility_tiny_url WHERE id=:id",
                                     |mut row| TinyUrl {
                                         id: row.take(0).unwrap(),
                                         url_payload: row.take(1).unwrap(),
                                     }, params!("id" => id)).ok_or_else(|| UtilityFailure::InvalidInput)
    }

    fn get_tiny_url_id_by_payload(&self, db_main: &mut impl Select, payload: &String) -> Result<u32, UtilityFailure> {
        return db_main.select_wparams_value("SELECT id FROM utility_tiny_url WHERE url_payload=:url_payload",
                                            |mut row| row.take::<u32, usize>(0).unwrap(),
        params!("url_payload" => payload.clone())).ok_or_else(|| UtilityFailure::InvalidInput);
    }
